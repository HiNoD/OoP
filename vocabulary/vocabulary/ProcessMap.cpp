...
{
	cout << "В словарь были внесены изменения. Введите Y\n для сохранения";
	char agreement;
	cin >> agreement;
...
void UserIntaraction(Dictionary &dictionary, std::string const &nameDictionary)
{
	...
	while (receiptSearchKey(keyToFind))
	{
		...
		{
			cout << "Неизвестное слово " << keyToFind << ". Введите перевод или пустую строку для отказа." << endl;
			...
			{
				cout << "Слово " << keyToFind << " проигнорировано.\n";
			}
		}
	}
	if (wasAdd)
